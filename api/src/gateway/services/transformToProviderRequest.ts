// @ts-nocheck
// Type checking suppressed in this file because upstream code uses pervasive
// dynamic indexing patterns that conflict with our `noUncheckedIndexedAccess`
// strictness. Real type validation runs under `tsconfig.gateway.json`, which
// uses relaxed settings matching upstream. Re-enable once project references
// land (see docs/MANAGED-GATEWAY-PLAN.md).
import { GatewayError } from '../errors/GatewayError.js';
import { AZURE_OPEN_AI, FIREWORKS_AI } from '../globals.js';
import ProviderConfigs from '../providers/index.js';
import { endpointStrings, ProviderConfig } from '../providers/types.js';
...
const transformToProviderRequestJSON = (
  ...
  if (!providerConfig) {
    throw new GatewayError(`Unsupported provider: ${provider}`);
  }
