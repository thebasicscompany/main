...
// Redis backend removed in this build (cache: false). Restore upstream
// `./backends/redis` and the `ioredis` dependency to re-enable.
import { createCloudflareKVBackend } from './backends/cloudflareKV.js';
...
export class CacheService {
  ...
  private createBackend(config: CacheConfig): CacheBackend {
    switch (config.backend) {
      ...
      case 'redis':
        throw new Error(
          'redis cache backend disabled in this build; restore upstream backends/redis.ts'
        );
